-- 20260917160000_fix_company_workstation_isolation_and_clocking.sql
-- Fix company workstation isolation and multi-tenant clocking resolution

-- 1. Correct misplaced workstations and update default coordinates
update public.company_workstations
set company_id = 'ce43d3b4-494d-401d-8476-3168465dbc71'
where id = '6e0fc684-8bed-4819-aa6b-432eb663e1c3';

update public.company_workstations
set latitude = -25.9085673,
    longitude = 29.2505837,
    address = coalesce(address, 'Emalahleni, Mpumalanga')
where company_id = 'ce43d3b4-494d-401d-8476-3168465dbc71'
  and name = 'Emalahleni Practice'
  and latitude = -26.2041;

delete from public.company_workstations
where company_id = 'c096fb54-7018-43cb-8a4d-72f67004c785'
  and name ilike '%Alora%';

-- 2. Drop obsolete function signatures to prevent PGRST202 overload ambiguities
drop function if exists public.upsert_company_workstation(uuid, text, text, numeric, numeric, integer);
drop function if exists public.record_employee_time_event(public.clock_event_type, jsonb, uuid, time);

-- 3. Rewrite upsert_company_workstation with explicit target_company_id support
create or replace function public.upsert_company_workstation(
  target_workstation_id uuid,
  workstation_name text,
  workstation_address text,
  workstation_latitude numeric,
  workstation_longitude numeric,
  workstation_radius_meters integer,
  target_company_id uuid default null
)
returns uuid
language plpgsql
security definer
set search_path = public
as $$
declare
  resolved_company_id uuid;
  saved_workstation_id uuid;
begin
  resolved_company_id := target_company_id;

  -- If target_workstation_id is provided, resolve from existing record
  if resolved_company_id is null and target_workstation_id is not null then
    select company_id
      into resolved_company_id
    from public.company_workstations
    where id = target_workstation_id;
  end if;

  -- Fallback to user company
  if resolved_company_id is null then
    select company_id
      into resolved_company_id
    from public.users
    where auth_user_id = auth.uid()
      and status = 'active'
      and deleted_at is null
    order by created_at asc
    limit 1;
  end if;

  if resolved_company_id is null then
    raise exception 'No active company could be determined for this workstation';
  end if;

  if not public.has_any_company_role(resolved_company_id, array['owner', 'hr_admin']::public.app_role[]) then
    raise exception 'Only company admins can manage workstations';
  end if;

  if target_workstation_id is null then
    insert into public.company_workstations (
      company_id,
      name,
      address,
      latitude,
      longitude,
      radius_meters
    )
    values (
      resolved_company_id,
      btrim(workstation_name),
      nullif(btrim(coalesce(workstation_address, '')), ''),
      workstation_latitude,
      workstation_longitude,
      workstation_radius_meters
    )
    returning id into saved_workstation_id;
  else
    update public.company_workstations
    set name = btrim(workstation_name),
        address = nullif(btrim(coalesce(workstation_address, '')), ''),
        latitude = workstation_latitude,
        longitude = workstation_longitude,
        radius_meters = workstation_radius_meters,
        is_active = true,
        deleted_at = null,
        updated_at = now()
    where id = target_workstation_id
      and company_id = resolved_company_id
    returning id into saved_workstation_id;

    if saved_workstation_id is null then
      -- Fallback if id exists under resolved_company_id regardless of former soft-delete
      update public.company_workstations
      set name = btrim(workstation_name),
          address = nullif(btrim(coalesce(workstation_address, '')), ''),
          latitude = workstation_latitude,
          longitude = workstation_longitude,
          radius_meters = workstation_radius_meters,
          is_active = true,
          deleted_at = null,
          updated_at = now()
      where id = target_workstation_id
      returning id into saved_workstation_id;
    end if;
  end if;

  return coalesce(saved_workstation_id, target_workstation_id);
end;
$$;

grant execute on function public.upsert_company_workstation(uuid, text, text, numeric, numeric, integer, uuid) to authenticated;

-- 4. Rewrite record_employee_time_event with target_company_id and employee resolution
create or replace function public.record_employee_time_event(
  requested_event public.clock_event_type,
  device_metadata jsonb default '{}'::jsonb,
  workstation_id uuid default null,
  requested_at time default null,
  target_company_id uuid default null
)
returns public.time_entries
language plpgsql
security definer
set search_path = public
as $$
declare
  actor public.users%rowtype;
  employee public.employees%rowtype;
  company_timezone text;
  local_now timestamp;
  local_date date;
  local_time time;
  target_period_id uuid;
  target_timesheet_id uuid;
  entry public.time_entries%rowtype;
  current_clock_event_id uuid;
  resolved_company_id uuid;
  picked_workstation public.company_workstations%rowtype;
  calculated_distance numeric;
  calculated_geofence_status text;
  enriched_metadata jsonb;
  event_latitude numeric;
  event_longitude numeric;
  recorded_local_time time;
begin
  resolved_company_id := target_company_id;

  -- If workstation_id provided and company not specified, resolve from workstation
  if resolved_company_id is null and workstation_id is not null then
    select company_id
      into resolved_company_id
    from public.company_workstations
    where id = workstation_id;
  end if;

  -- Resolve actor scoped to company when possible
  if resolved_company_id is not null then
    select *
      into actor
    from public.users
    where auth_user_id = auth.uid()
      and company_id = resolved_company_id
      and status = 'active'
      and deleted_at is null
    limit 1;
  end if;

  -- Fallback if not found for specific company
  if actor.id is null then
    select *
      into actor
    from public.users
    where auth_user_id = auth.uid()
      and status = 'active'
      and deleted_at is null
      and employee_id is not null
    order by created_at asc
    limit 1;
  end if;

  if actor.id is null then
    select *
      into actor
    from public.users
    where auth_user_id = auth.uid()
      and status = 'active'
      and deleted_at is null
    order by created_at asc
    limit 1;
  end if;

  if not found or actor.id is null then
    raise exception 'No active account is linked to this login';
  end if;

  if resolved_company_id is null then
    resolved_company_id := actor.company_id;
  end if;

  -- Resolve employee record
  if actor.employee_id is not null then
    select *
      into employee
    from public.employees
    where id = actor.employee_id
      and company_id = resolved_company_id
      and employment_status = 'active'
      and deleted_at is null
    for update;
  end if;

  -- Fallback: resolve employee by email or user_id in target company
  if employee.id is null then
    select *
      into employee
    from public.employees
    where company_id = resolved_company_id
      and (user_id = actor.id or email = actor.email)
      and employment_status = 'active'
      and deleted_at is null
    order by created_at asc
    limit 1
    for update;
  end if;

  if employee.id is null then
    raise exception 'Active employee record could not be found for this company';
  end if;

  -- Extract coordinates from device metadata
  if device_metadata is not null and device_metadata ? 'location' and device_metadata->'location' is not null then
    event_latitude := nullif(device_metadata->'location'->>'latitude', '')::numeric;
    event_longitude := nullif(device_metadata->'location'->>'longitude', '')::numeric;
  end if;

  -- Resolve the workstation: picked one wins, otherwise the active assignment
  if workstation_id is not null then
    select *
      into picked_workstation
    from public.company_workstations ws
    where ws.id = workstation_id
      and ws.company_id = resolved_company_id
      and ws.is_active
      and ws.deleted_at is null;

    if picked_workstation.id is null then
      -- Allow matching without is_active check if workstation belongs to company
      select *
        into picked_workstation
      from public.company_workstations ws
      where ws.id = workstation_id
        and ws.company_id = resolved_company_id;
    end if;
  else
    select workstation.*
      into picked_workstation
    from public.employee_workstation_assignments assignment
    join public.company_workstations workstation
      on workstation.id = assignment.workstation_id
     and workstation.company_id = assignment.company_id
     and workstation.is_active
     and workstation.deleted_at is null
    where assignment.company_id = resolved_company_id
      and assignment.employee_id = employee.id
      and assignment.is_active
      and assignment.deleted_at is null
      and current_date between assignment.effective_from and coalesce(assignment.effective_to, current_date)
    order by assignment.created_at desc
    limit 1;
  end if;

  if picked_workstation.id is null then
    calculated_geofence_status := 'no_workstation';
  elsif event_latitude is null or event_longitude is null then
    calculated_geofence_status := 'no_location';
  else
    calculated_distance := public.distance_between_coordinates_meters(
      event_latitude,
      event_longitude,
      picked_workstation.latitude,
      picked_workstation.longitude
    );
    calculated_geofence_status := case
      when calculated_distance <= picked_workstation.radius_meters then 'in_range'
      else 'out_of_range'
    end;
  end if;

  enriched_metadata := jsonb_set(
    coalesce(device_metadata, '{}'::jsonb),
    '{geofence}',
    jsonb_build_object(
      'status', calculated_geofence_status,
      'distance_meters', calculated_distance,
      'workstation_id', picked_workstation.id,
      'workstation_name', picked_workstation.name,
      'radius_meters', picked_workstation.radius_meters
    ),
    true
  );

  select timezone
    into company_timezone
  from public.companies
  where id = resolved_company_id;

  company_timezone := coalesce(nullif(company_timezone, ''), 'Africa/Johannesburg');
  local_now := now() at time zone company_timezone;
  local_date := local_now::date;
  local_time := local_now::time;
  recorded_local_time := local_time;

  select id
    into target_period_id
  from public.payroll_periods
  where company_id = resolved_company_id
    and local_date between period_start and period_end
    and status in ('open', 'reopened')
    and deleted_at is null
  order by period_start desc
  limit 1;

  select *
    into entry
  from public.time_entries
  where company_id = resolved_company_id
    and employee_id = employee.id
    and work_date = local_date
    and deleted_at is null
  order by created_at desc, id desc
  limit 1
  for update;

  if not found then
    select id
      into target_timesheet_id
    from public.timesheets
    where company_id = resolved_company_id
      and employee_id = employee.id
      and (
        (target_period_id is not null and payroll_period_id = target_period_id)
        or (target_period_id is null and payroll_period_id is null and status = 'draft')
      )
      and status in ('draft', 'rejected')
      and deleted_at is null
    order by created_at desc
    limit 1;

    if target_timesheet_id is null then
      insert into public.timesheets (
        company_id,
        employee_id,
        payroll_period_id,
        status
      )
      values (
        resolved_company_id,
        employee.id,
        target_period_id,
        'draft'
      )
      returning id into target_timesheet_id;
    end if;

    insert into public.time_entries (
      company_id,
      timesheet_id,
      employee_id,
      work_date,
      workstation_id,
      status,
      device_metadata
    )
    values (
      resolved_company_id,
      target_timesheet_id,
      employee.id,
      local_date,
      picked_workstation.id,
      'draft',
      enriched_metadata
    )
    returning * into entry;
  end if;

  if requested_event = 'clock_in' then
    if entry.clock_in is not null and entry.clock_out is null then
      raise exception 'You are already clocked in. Clock out or start lunch first.';
    end if;

    if entry.clock_out is not null then
      if requested_at is not null then
        if requested_at <= entry.clock_out then
          raise exception 'New shift start time must be after previous shift clock out (%)', entry.clock_out;
        end if;
      else
        if local_time <= entry.clock_out then
          raise exception 'Cannot start a new shift before previous shift clock out (%)', entry.clock_out;
        end if;
      end if;

      insert into public.time_entries (
        company_id,
        timesheet_id,
        employee_id,
        work_date,
        workstation_id,
        status,
        device_metadata
      )
      values (
        resolved_company_id,
        entry.timesheet_id,
        employee.id,
        local_date,
        coalesce(picked_workstation.id, entry.workstation_id),
        'draft',
        enriched_metadata
      )
      returning * into entry;
    end if;

    if requested_at is not null then
      recorded_local_time := requested_at;
    end if;

    update public.time_entries
    set clock_in = recorded_local_time,
        workstation_id = coalesce(picked_workstation.id, entry.workstation_id),
        device_metadata = enriched_metadata,
        updated_at = now()
    where id = entry.id
    returning * into entry;

  elsif requested_event = 'lunch_start' then
    if entry.clock_in is null then
      raise exception 'Cannot start lunch before clocking in';
    end if;
    if entry.clock_out is not null then
      raise exception 'Shift is already complete';
    end if;
    if entry.lunch_start is not null then
      raise exception 'Lunch already started';
    end if;

    update public.time_entries
    set lunch_start = recorded_local_time,
        workstation_id = coalesce(picked_workstation.id, entry.workstation_id),
        device_metadata = enriched_metadata,
        updated_at = now()
    where id = entry.id
    returning * into entry;

  elsif requested_event = 'lunch_end' then
    if entry.lunch_start is null then
      raise exception 'Cannot end lunch before starting lunch';
    end if;
    if entry.lunch_end is not null then
      raise exception 'Lunch already ended';
    end if;

    update public.time_entries
    set lunch_end = recorded_local_time,
        workstation_id = coalesce(picked_workstation.id, entry.workstation_id),
        device_metadata = enriched_metadata,
        updated_at = now()
    where id = entry.id
    returning * into entry;

  elsif requested_event = 'clock_out' then
    if entry.clock_in is null then
      raise exception 'Cannot clock out before clocking in';
    end if;
    if entry.clock_out is not null then
      raise exception 'Already clocked out for today';
    end if;

    update public.time_entries
    set clock_out = recorded_local_time,
        workstation_id = coalesce(picked_workstation.id, entry.workstation_id),
        device_metadata = enriched_metadata,
        updated_at = now()
    where id = entry.id
    returning * into entry;

  elsif requested_event = 'switch_workstation' then
    if picked_workstation.id is null then
      raise exception 'Choose a valid workstation to switch to';
    end if;

    update public.time_entries
    set workstation_id = picked_workstation.id,
        device_metadata = enriched_metadata,
        updated_at = now()
    where id = entry.id
    returning * into entry;
  end if;

  insert into public.time_clock_events (
    company_id,
    time_entry_id,
    employee_id,
    event_type,
    event_at,
    local_work_date,
    local_event_time,
    latitude,
    longitude,
    accuracy_meters,
    workstation_id,
    distance_meters,
    geofence_status
  )
  values (
    resolved_company_id,
    entry.id,
    employee.id,
    requested_event,
    coalesce(
      nullif(device_metadata->'location'->>'captured_at', '')::timestamptz,
      now()
    ),
    local_date,
    recorded_local_time,
    event_latitude,
    event_longitude,
    nullif(device_metadata->'location'->>'accuracy', '')::numeric,
    picked_workstation.id,
    calculated_distance,
    calculated_geofence_status
  )
  returning id into current_clock_event_id;

  return entry;
end;
$$;

grant execute on function public.record_employee_time_event(public.clock_event_type, jsonb, uuid, time, uuid) to authenticated;
