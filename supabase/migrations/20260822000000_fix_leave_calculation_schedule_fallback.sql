-- Fix leave calculation to use employee/work schedule fallback for ALL days,
-- not just days without timesheets (absent days).
--
-- The previous migration (20260808220000) only fell back to the employee's
-- work_schedule_id and company-wide schedule when the day had no timesheet.
-- This caused days with timesheets but outside assignment effective windows
-- to be treated as non-working days (0 hours), blocking leave requests.
--
-- Now: whenever an active assignment lookup fails to find a schedule day,
-- we fall back to employee.work_schedule_id, then company-wide schedule.
-- The 'absent' flag is retained only for informational purposes.
...
begin
  ...
  loop
    ...
    if assigned_rule_count > 0 then
      ...
      if not found then
        -- Fallback to employee's default work schedule, then company-wide schedule
        -- This now runs regardless of whether the day has a timesheet (absent flag)
      ...
      if not found or not coalesce(schedule_day.is_working_day, false) then
        -- Fallback to company-wide schedule
        select sd.*
          into schedule_day
        from public.work_schedules schedules
        join public.schedule_days sd
