-- Prevent clocking in at a time that clashes with another shift on the same day.
--
-- An employee may have several shifts in one day, but a new clock-in time must
-- not land inside an existing shift window (the same start time, or a time
-- before another completed shift has ended, is rejected). Back-to-back shifts
-- are still fine (e.g. 08:00-12:00 then 12:00-16:00).
...
begin
  ...
  if requested_event = 'clock_in' then
    ...
    -- Reject clock-ins that land inside a shift that is already recorded today.
    if exists (
      select 1
      from public.time_entries te
      where te.company_id = employee.company_id
        and te.employee_id = employee.id
        and te.work_date = local_date
        and te.deleted_at is null
        and te.clock_in <= recorded_local_time
        and (te.clock_out is null or te.clock_out > recorded_local_time)
    ) then
      raise exception 'This clock-in time overlaps another shift today';
    end if;
