-- Clocking overhaul: employees pick a workstation for every clock event
-- (defaulting to their active assignment), can switch workstations mid-shift,
-- and out-of-range events stay allowed but are flagged.
--
-- 1. Add the 'switch_workstation' clock event type.
-- 2. Replace record_employee_time_event with a version that accepts an optional
--    workstation_id. The geofence is validated against the picked workstation.
...
begin
  if not exists (
    ...
    where t.typname = 'clock_event_type'
      and e.enumlabel = 'switch_workstation'
  ) then
    alter type public.clock_event_type add value 'switch_workstation';
  end if;
end $$;

-- Drop the old 2-argument signature so PostgREST resolves unambiguously.
drop function if exists public.record_employee_time_event(public.clock_event_type, jsonb);
...
grant execute on function public.record_employee_time_event(public.clock_event_type, jsonb, uuid) to authenticated;
